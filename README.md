...
这是一个基于 ESP32-S3、STM32U5 的小型自动化物流分拣系统。项目核心是利用运行在 PC 上的 PaddleOCR 模型进行高速视觉识别，并将识别到的包裹关键字通过局域网发送给 ESP32-S3。ESP32-S3 采用 FreeRTOS 实时操作系统，高效、稳定地管理多个硬件外设（传送带、分拣舵机、卸货舵机、传感器等），并根据指令完成包裹的分拣、暂存， STM32U5 小车进行取货。
...
*   [部署与使用](#部署与使用)
    ...
    *   [第五步：STM32U5 小车固件](#第五步STM32U-小车固件)
*   [运行流程](#运行流程)
...
3.  **搬运小车 (STM32U5 + ESP8266)**：
    *   **角色**：最终执行取货和搬运任务的物理执行器。
    *   **技术**：采用“主控+通信”双芯片架构。**STM32U5** 作为主控，负责实时循迹算法和电机控制；**ESP8266** 作为通信模块，通过 TCP 转串口的方式与主控系统连接。
    *   **通信**：ESP8266 作为一个 TCP 服务器运行，等待 ESP32-S3 的连接和指令，并将指令透传给 STM32U5。
...
小车固件分为两部分，需要分别烧录。详细的技术细节和引脚定义请参考 `STM32U5_Car_Firmware/README.md` 文件。

1.  **烧录 STM32U5 主控固件**:
    *   使用 Keil MDK 打开 `STM32U5_Car_Firmware/MDK-ARM/` 目录下的工程文件。
    *   编译并使用 ST-Link 烧录。

2.  **烧录 ESP8266 通信固件**:
    *   使用 Arduino IDE 打开小车WIFI模块的 ESP8266 代码。
    *   **修改代码中的 WiFi 账号和密码**。
    *   编译并烧录。
